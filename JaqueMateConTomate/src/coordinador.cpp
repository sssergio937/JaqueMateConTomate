...
void Coordinador::dibuja()
{
	...
	case RANKING: {
		...
		int y = 12;
		for (const auto& j : ranking) {
			std::string nivelStr = (j.nivel == 1) ? "Tomatito" : (j.nivel == 2) ? "Señor Tomate" : "MAESTRO Tomate";
	...
	case PEDIR_DATOS:
		...
		if (pedirDatosPaso == 0) {
			...
			std::string difText = (dificultadMaquina == 1) ? "Tomatito" : (dificultadMaquina == 2) ? "Señor Tomate" : "MAESTRO Tomate";
	...
	case JUEGO1J:
		...
		mundo.dibuja();
		//si es el turno de la máquina, ejecuta su movimiento automáticamente
	...
	case CREDITOS:
		...
		ETSIDI::printxy("Sergio: Recolectó y cosechó las piezas", -20, 7);
		ETSIDI::printxy("Oscar: Plantó y regó las piezas", -20, 5);
		ETSIDI::printxy("GRACIAS POR JUGAR", -20, 1);
		ETSIDI::printxy("ESC para volver al menú", 11, -16);
		break; 
	case FIN:
		if (!reproducido) {
			//system("start /wait wmplayer.exe \"C:\\UNIVERSIDAD\\3º Carrera\\Informatica Industrial\\JaqueMateConTomate-cambios (2)\\JaqueMateConTomate-cambios\\JaqueMateConTomate IA semichunga\\bin\\JaqueMateConTomate.mp4\"");
